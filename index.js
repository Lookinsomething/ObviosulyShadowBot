...
client.on("message", async message => {
  ...
  let profile = await leveling.Fetch(message.author.id);
    ...
    if(profile.xp + 15 > 150) {
            ...
            message.channel.send(`Congratulations ${message.author.username}, you just advanced to level ${profile.level + 1}! Unlock eash level at 150 exp!`)
  ...
  if(command === "help") {
    ...
    .addField("!deletewarns <@user> [reason]", "delete all warns from the user!")
    ...
    .addField("Leveling", "A system to track down how active you are! Basically you get level up when you talk a lot in the server")
  ...
  if(command === "warn") {
    let db = require('quick.db') // npm i quick.db
        ...
        if(!user) return message.channel.send('Which user would you like to warn? Please mention the user by the way.') // If User Is Not Provided
    ...
    if(command === "deletewarns") {
      let user = message.mentions.members.first()
        if(!user) return message.channel.send('Whom Do You Want To Unwarn?') // If No User Is Provided
    ...
    if(command === "meme") {
      ...
      let meme = await fetch('https://meme-api.herokuapp.com/gimme').then((res) => res.json()) // From Where BOT Will Get Memes
    ...
    if(command === "bal") {
      ...
      if(!user) return message.channel.send("Please state a user to check the balance!")
    ...
    if(command === "buy") {
            if(args[0].toLowerCase() === 'house') { // You Can Keep Any Name
            const user = message.member
            const amount = 1000000 // Amount Of Laptop: $10,000
            const bal = db.fetch(`money_${user.id}`) // Get User's Money In Wallet

            if(bal <amount) { // If Balance In Wallet Is Less Then Amount Of House
                return message.reply(`You Don't Have Enough Money(*$1,000,000*) In Wallet To Buy House!`)
            } else {
                const embed = new Discord.MessageEmbed()
                .setColor('RANDOM')
...
Default CoolDown Is **24 Hours(1 Day)**`)
        ...
        if(command === "inv") {
          if(db.get(message.author.id) === null) return message.channel.send("You have nothing lol")
          
          message.channel.send(`Your inventory: ${db.get(message.author.id)}`)
        }
    }
})
